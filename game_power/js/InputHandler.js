...
if (key === 'f') {
    this.game.updateZoom(this.game.zoom - 0.5);
            ...
            if (key === 's') { 
                ...
                // القوة الخارقة بعد 0.5 ثانية
                if (this.game.attackHoldTimeout) clearTimeout(this.game.attackHoldTimeout);
                this.game.attackHoldTimeout = setTimeout(() => {
                    if (this.game.isAttackPressed) {
        ...
        window.addEventListener('keyup', (e) => {
            ...
            // إعادة تعيين حالة زر S
            if (key === 's') {
                this.game.isAttackPressed = false;
                if (this.game.attackHoldTimeout) {
                    clearTimeout(this.game.attackHoldTimeout);
                    this.game.attackHoldTimeout = null;
                }
            }
